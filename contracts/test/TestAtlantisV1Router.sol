...
contract TestAtlantisV1Router is IAtlantisV1SwapCallback {
    ...
    function swapForExact0Multi(
        ...
        pools[0] = poolInput;
        IAtlantisV1Pool(poolOutput).swap(
            recipient,
            false,
            -amount0Out.toInt256(),
    ...
    function AtlantisV1SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IAtlantisV1Pool(msg.sender).token0() : IAtlantisV1Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IAtlantisV1Pool(pools[0]).token1();
            IAtlantisV1Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IAtlantisV1Pool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IAtlantisV1Pool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
